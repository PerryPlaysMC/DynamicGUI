package dev.perryplaysmc.mines.utils.guis.utils.guitypes;

import example.plugin.PluginMain;
...
import dev.perryplaysmc.mines.utils.guis.utils.Version;
...
public class BetterListGUI extends BetterGUI {

    private BetterGUI mainPage;
    private BetterListGUI mainListPage;
    ...
    public BetterListGUI(String name, int size) {
        super(name, size);
    ...
    public BetterListGUI(BetterGUI mainPage, String name, int size) {
        this(name, size);
        if(mainPage!=null)
            if(mainPage instanceof BetterListGUI)
                this.mainListPage = (BetterListGUI) mainPage;
            else
    ...
    /**
     ...
     @param removeDelay
     */
    public BetterListGUI setRemoveDelay(int removeDelay) {
        this.removeDelay = removeDelay;
        return this;
    }

    @Override
    public BetterListGUI addFiller(FillerType type, ItemBuilder... replace) {
        int size = getSize();
        int loopSize = size;
        int start = 0;
        switch(type) {
    ...
    @Override
    public BetterListGUI setItem(int slot, ItemStack item) { return setItem(slot, new ItemBuilder(item)); }

    @Override
    public BetterListGUI setItem(int slot, ItemBuilder item) {
        items = pages.get(1);
    ...
    public BetterListGUI setItem(int page, int slot, ItemBuilder item) {
    ...
    @Override
    public BetterListGUI addItem(ItemStack... itemstacks) {
        int slot = 0;
        int page = 1;
        items = pages.get(page);
        for(ItemStack item : itemstacks) {
            ...
            while(items.containsKey(slot) || fillers.containsKey(slot)) {
                slot++;
                if(slot == getSize()) {
                    slot = 0;
                    page++;
    ...
    @Override
    public BetterListGUI addItem(ItemBuilder... items) {
        addItemLeftover(items);
        return this;
    }

    @Override
    public BetterListGUI addItem(ItemBuilder item) {
     ...
     * Clears the slot
     */
    public BetterGUI clear(int slot) {
     ...
     * @return
     */
    public BetterListGUI removeItem(ItemStack item) {
     ...
     * @return
     */
    public BetterListGUI removeItem(Material item, int amount) {
        int amt = amount;
        HashMap<Integer, List<Integer>> toRemove = new HashMap<>();
        for(int page : pages.keySet()) {
            HashMap<Integer, ItemStack> items = pages.get(page);
     ...
     * @return
     */
    public BetterListGUI removeItem(Material item) {
        for(int page : pages.keySet()) {
            HashMap<Integer, ItemStack> items = pages.get(page);
            for(Integer slot : items.keySet()) {
                ItemStack slotItem = items.get(slot);
    ...
    public BetterListGUI setNextPageItem(int slot, ItemBuilder item)  {
        if(item == null) return this;
        next = new TurnPage(slot, item.replaceInLore("{next}", (currentPage+1) > getPages() ? "You're on the last page!" : (currentPage+1)));
        return this;
    }

    public BetterListGUI setPreviousPageItem(int slot, ItemBuilder item)  {
        if(item == null) return this;
