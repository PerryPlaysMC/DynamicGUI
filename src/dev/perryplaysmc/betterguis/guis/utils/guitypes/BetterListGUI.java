package dev.perryplaysmc.betterguis.guis.utils.guitypes;

import dev.perryplaysmc.betterguis.PluginMain;
...
import dev.perryplaysmc.betterguis.guis.utils.Version;
...
public class BetterListGUI extends BetterGUI {
    ...
    private PluginMain main = PluginMain.getPlugin(PluginMain.class);
    ...
    public BetterListGUI setNextPage(int slot, ItemBuilder item)  {
        if(item == null) return this;
        next = new TurnPage(slot, item.replaceInLore("{next}", (currentPage+1) > getPages() ? "You're on the last page!" : (currentPage+1)));
        return this;
    }

    public BetterListGUI setPreviousPage(int slot, ItemBuilder item)  {
        if(item == null) return this;
